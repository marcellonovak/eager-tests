...
# GRIB2 Time Series Processor v2
...
for file in files:
    ...
    with pygrib.open(file) as current_grib:
        ...
        headers = [columns_cache[sub_grid] for sub_grid in all_grids]
        ...
        for i in range(30): #latitudes.shape[0]):
            for j in range(30): #latitudes.shape[1]):
                ...
                # If the file is already processed in this session...
                if outfile in existing_files:
                    # ...Just append the new data to the file
                    new_data.to_csv(outfile, mode='a', header=False)
                # If the file is not processed in this session...
                else:
                    # ...Check if it exists, and if it does, append the new data to the file
                    if os.path.isfile(outfile):
                        new_data.to_csv(outfile, mode='a', header=False)
                    else:
                        new_data.to_csv(outfile, mode='w', columns=headers)
